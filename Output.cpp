...
int Output::GetCellStartX(const CellPosition &cellPos) const
{
	...
	int cellX = cellPos.HCell() * UI.CellWidth;
	return cellX;
}
...
int Output::GetCellStartY(const CellPosition &cellPos) const
{
	...
	int cellY = UI.ToolBarHeight + cellPos.VCell() * UI.CellHeight;
	return cellY;
}
...
void Output::CreateDesignModeToolBar() const
{
	...
	MenuItemImages[ITM_ADD_SNAKE] = "images\\snake.jpg";
	MenuItemImages[ITM_ADD_CARD] = "images\\add_card.jpg";

	/// TODO: Prepare images for each menu item and add it to the list
	MenuItemImages[ITM_COPY_CARD] = "images\\copy.jpg";
	MenuItemImages[ITM_CUT_CARD] = "images\\cutting.jpg";
	MenuItemImages[ITM_PASTE_CARD] = "images\\paste.jpg";
	MenuItemImages[ITM_EDIT_CARD] = "images\\edit.jpg";
	MenuItemImages[ITM_DELETE_GAME_OBJ] = "images\\delete.jpg";
	MenuItemImages[ITM_SAVE_GRID] = "images\\save_grid.jpg";
	MenuItemImages[ITM_OPEN_GRID] = "images\\grid.jpg";
	MenuItemImages[ITM_SWITCH_TO_PLAY_MODE] = "images\\switch_to_play_mode.jpg";
	MenuItemImages[ITM_EXIT_DESIGN_MODE] = "images\\exit.jpg";
...
void Output::CreatePlayModeToolBar() const
{
	...
	/// TODO: Prepare images for each menu item and add it to the list
	MenuItemImages[ITM_INPUT_DICE_VAL] = "images\\input_dice_value.jpg";
...
void Output::PrintPlayersInfo(string info)
{
	...
	pWind->GetStringSize(w, h, info); // from the cmu graphics library
	...
	pWind->DrawString(x, y, info); // using the current windowpointer to access the functions of the instance
}
...
void Output::DrawLadder(const CellPosition &fromCell, const CellPosition &toCell) const
{
	...
	int local_numberOfCells = fromCell.VCell() - toCell.VCell();
	for (int i = 0; i < local_numberOfCells; i++)
	{
		pWind->SetPen(UI.LadderColor, UI.LadderlineWidth);
		int curr_y = fromStartY - (i * UI.CellHeight);
		pWind->DrawLine(x12, curr_y, x34, curr_y);
	}
}
...
void Output::DrawSnake(const CellPosition &fromCell, const CellPosition &toCell) const
{
	...
	pWind->DrawLine(x12, y1, x12, y2); // given before just raed the code well
	...
	int xi[] = {h2, x12, h1, x12};
	int yi[] = {y1, v1, y1, v2};
	pWind->DrawPolygon(xi, yi, 4);
	//        Check the snakes drawn in the project document and draw it the same way
}
